...
function GavetaDossie({ sku, publicado, fechar, recarregar }: { sku: string; publicado: boolean; fechar: () => void; recarregar: () => void }) {
  ...
  const carregar = () => {
    setLoading(true);
    axios.get(`/api/v1/dashboard/dossie?sku=${encodeURIComponent(sku)}`).then((r) => setD(r.data)).finally(() => setLoading(false));
  };
  useEffect(() => { carregar(); }, [sku]);
  ...
  const CAMADAS = [
    { key: 'ia', label: 'IA', cor: '#94a3b8' },
    { key: 'topdown', label: 'TopDown', cor: '#6366f1' },
    { key: 'bottomup', label: 'BottomUp', cor: '#8b5cf6' },
    { key: 'meta', label: 'Meta', cor: '#a855f7' },
    { key: 'supply', label: 'Supply', cor: '#d946ef' },
  ];

  return (
    <>
      <div className="fixed inset-0 bg-slate-900/20 z-30" onClick={fechar} />
      <div className="fixed right-0 top-0 bottom-0 w-full max-w-lg bg-white z-40 shadow-2xl overflow-y-auto" style={{ fontVariantNumeric: 'tabular-nums' }}>
        <div className="sticky top-0 bg-white border-b border-slate-100 px-5 py-4 flex items-start justify-between">
          <div className="min-w-0">
            <div className="text-[10px] font-black uppercase tracking-widest text-indigo-500">Cenários & Dossiê</div>
        ...
        {loading ? (
          ...
          <div className="p-5 space-y-6">
            ...
            <div>
              <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Comparador de cenários · adote uma camada como Final</div>
              <div className="space-y-3">
                {(d?.cenarios || []).map((c: any) => {
                  const maxV = Math.max(c.ia, c.topdown, c.bottomup, c.meta, c.supply, c.final, 1);
                  return (
                    <div key={c.mes} className="rounded-xl border border-slate-100 p-3">
                      <div className="flex items-center justify-between mb-2">
                        <div className="text-xs font-black text-slate-700">{mesLabel(c.mes)}</div>
                        {c.dispersao >= 0.2 && (
                          <div className="text-[10px] font-bold text-amber-600 flex items-center gap-1">
                            <AlertTriangle className="w-3 h-3" /> etapas divergem {fmtPct(c.dispersao)}
                          </div>
                        )}
                      </div>
                      <div className="space-y-1.5">
                        {CAMADAS.map((cam) => (
                          <div key={cam.key} className="flex items-center gap-2">
                            <div className="w-16 text-[10px] font-bold text-slate-500 shrink-0">{cam.label}</div>
                            <div className="flex-1 h-4 bg-slate-50 rounded overflow-hidden">
                              <div className="h-full rounded" style={{ width: `${(c[cam.key] / maxV) * 100}%`, background: cam.cor }} />
                            </div>
                            <div className="w-14 text-right text-[11px] font-bold text-slate-600 shrink-0">{fmtCx(c[cam.key])}</div>
                            {!publicado && (
                              <button onClick={() => adotar(c.mes, cam.key)}
                                className="text-[10px] font-black text-indigo-600 hover:bg-indigo-50 rounded px-1.5 py-0.5 shrink-0" title={`Adotar ${cam.label} como Final`}>
                                adotar
                              </button>
                            )}
                          </div>
                        ...
                        <div className="flex items-center gap-2 pt-1 border-t border-slate-100 mt-1">
                          <div className="w-16 text-[10px] font-black text-slate-900 shrink-0 flex items-center gap-1"><Check className="w-3 h-3 text-emerald-600" />Final</div>
                          <div className="flex-1 h-4 bg-emerald-50 rounded overflow-hidden">
                            <div className="h-full rounded bg-emerald-500" style={{ width: `${(c.final / maxV) * 100}%` }} />
                          </div>
                          <div className="w-14 text-right text-[11px] font-black text-emerald-700 shrink-0">{fmtCx(c.final)}</div>
                          <div className="w-10" />
                        </div>
                      </div>
                    </div>
            ...
            {d?.plurianual && (
              <div>
                ...
                <div className="space-y-1">
                  <div className="grid grid-cols-4 gap-2 px-3 text-[9px] font-black uppercase tracking-widest text-slate-300">
                    <div>Ano</div><div className="text-right">Volume</div><div className="text-right">PMV</div><div className="text-right">Clientes</div>
                  </div>
