...
export default function DemandaSupply() {
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50">
      ...
      <div className="flex-1 min-h-0">
        {aba === 'geral' ? (
          <VisaoGeralMarketing prefixoApi="/api/v1/supply" />
        ) : (
          <PreenchimentoSupply />
        )}
      </div>
...
function PreenchimentoSupply() {
  ...
  const congeladaPropria: boolean = dados?.congelada_propria;
      ...
      cat.segmentos.forEach((seg: any) =>
        seg.skus.forEach((s: any) =>
          meses.forEach((m) => {
            const cel = s.meses[m];
            if (!cel) return;
            const v = valorCelula(s.sku, m, cel.supply);
            vol[m] += v;
            fat[m] += v * (cel.pmv || 0);
  ...
  const reabrirEtapa = async () => {
    if (!confirm('Reabrir Supply Review? A etapa voltará a ser editável.')) return;
    try {
      await axios.post('/api/v1/supply/reabrir', {});
      await carregar();
        ...
        <div className="px-6 pt-5 pb-3 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs font-medium text-slate-400">
              Ciclo {dados?.ciclo} · defina os volumes de Supply SKU a SKU para o próximo ciclo.
              ...
              >
                ...
                {meses.map((m) => {
                  ...
                  const fatCat = cat.segmentos.reduce((acc: number, seg: any) =>
                    acc + seg.skus.reduce((a: number, s: any) => {
                      const cel = s.meses[m]; if (!cel) return a;
                      return a + valorCelula(s.sku, m, cel.supply) * (cel.pmv || 0);
                    }, 0), 0);
                ...
                <div key={seg.nome} className="ml-4 mt-1">
                  ...
                  {seg.skus.map((s: any) => {
                    ...
                    const div = meses.reduce<'alta' | 'media' | null>((acc, m) => {
                      ...
                      const d = divergenciaIA(cel.ia, valorCelula(s.sku, m, cel.supply));
                    ...
                    return (
                      ...
                      <div
                        ...
                        {meses.map((m) => {
                          ...
                          const val = valorCelula(s.sku, m, cel.supply);
