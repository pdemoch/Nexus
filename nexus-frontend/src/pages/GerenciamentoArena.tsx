...
import {
  ChevronRight, ChevronDown, Save, Download, X,
...
/* =====================================================================
   DEMANDA COMERCIAL (Bottom-Up) — a gerência revê o Top-Down por cliente.
   Mesmo padrão do Marketing: tabela + gaveta + semáforo + totais vivos.
   Diferença: coluna editável é Bottom-Up; TopDown herdado é referência;
...
function divergencia(ref: number, val: number): 'alta' | 'media' | null {
  if (!ref || !val) return null;
  const d = Math.abs(val - ref) / Math.max(ref, val);
  if (d >= 0.4) return 'alta';
  if (d >= 0.2) return 'media';
  return null;
}

export default function DemandaComercial() {
  ...
  const topdownLiberado: boolean = dados?.topdown_liberado;
  ...
  if (!topdownLiberado) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center px-6">
        ...
        <h2 className="text-lg font-black text-slate-800">Aguardando a Diretoria</h2>
        <p className="text-sm text-slate-400 max-w-sm mt-1">
          A etapa Demanda Comercial abre quando o Top-Down for congelado pela Diretoria de Marketing.
        </p>
      </div>
    );
  }
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-slate-200">
        <div className="px-6 pt-5 pb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <a href="/api/v1/gerenciamento/exportar"
        ...
        <div className="px-6 pb-3 grid gap-2" style={{ gridTemplateColumns: `1fr repeat(${meses.length}, minmax(120px, 1fr))` }}>
          ...
          {meses.map((m) => {
            ...
            const vsOrc = orc > 0 ? (fat - orc) / orc : 0;
            return (
              <div key={m} className="bg-slate-50 rounded-lg px-3 py-2">
                ...
                <div className="text-[11px] font-bold text-emerald-600">{fmtRs(fat)}</div>
                {orc > 0 && (
                  <div className={`text-[10px] font-bold ${vsOrc >= 0 ? 'text-slate-400' : 'text-rose-500'}`}>
                    {vsOrc >= 0 ? '+' : ''}{fmtPct(vsOrc)} vs orç.
                  </div>
                )}
              </div>
            );
                ...
                <div key={seg.nome} className="ml-4 mt-1">
                  ...
                  {seg.skus.map((s: any) => {
                    const div = meses.reduce<'alta' | 'media' | null>((acc, m) => {
                      ...
                      const d = divergencia(cel.topdown, valorCelula(s.sku, m, cel.bottomup));
                      return d === 'alta' ? 'alta' : (d === 'media' && acc !== 'alta' ? 'media' : acc);
                    }, null);
                    const barra = div === 'alta' ? 'bg-rose-400' : div === 'media' ? 'bg-amber-400' : 'bg-transparent';
                    return (
                      <div key={s.sku} className="grid gap-2 px-3 py-1.5 items-center hover:bg-white rounded-lg group relative"
                        style={{ gridTemplateColumns: `1fr repeat(${meses.length}, minmax(120px, 1fr)) 40px` }}>
                        ...
                        {meses.map((m) => {
                          ...
                          return (
                            <div key={m} className="text-right">
                              ...
                              <div className="text-[9px] font-bold text-slate-300 pr-2">TD {fmtCx(cel.topdown)} · IA {fmtCx(cel.ia)}</div>
                            </div>
                          );
                        })}
...
/* GAVETA — idêntica ao padrão do Marketing, endpoint da própria etapa */
function GavetaDossie({ sku, fechar }: { sku: string; fechar: () => void }) {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get(`/api/v1/gerenciamento/dossie?sku=${encodeURIComponent(sku)}`)
