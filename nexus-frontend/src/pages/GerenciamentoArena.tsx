...
function PreenchimentoBottomUp() {
  ...
  const congelarEtapa = async () => {
    if (!confirm('Congelar Demanda Comercial? A etapa seguinte será liberada e esta ficará somente leitura.')) return;
    try {
      if (temEdicoes) await salvar();
      await axios.post('/api/v1/gerenciamento/congelar', {});
      await carregar();
    } catch (e: any) { alert(e?.response?.data?.detail || 'Falha ao congelar.'); }
  };

  const reabrirEtapa = async () => {
    if (!confirm('Reabrir Demanda Comercial? A etapa voltará a ser editável.')) return;
    try {
      await axios.post('/api/v1/gerenciamento/reabrir', {});
      await carregar();
      ...
      {aguardandoUpstream && (
        <div className="mx-6 mt-4 mb-2 flex items-center gap-3 rounded-xl bg-orange-50 border border-orange-200 px-4 py-3">
          ...
          <div>
            ...
            <div className="text-[11px] font-medium text-orange-600">Demanda Marketing ainda não congelou o plano deste ciclo. Os dados ficam visíveis, mas não editáveis até o bastão ser passado.</div>
          </div>
        </div>
      )}
