      ...
      cat.segmentos.forEach((seg: any) =>
        seg.skus.forEach((s: any) =>
          meses.forEach((m) => {
            const cel = s.meses[m];
            if (!cel) return;
            const v = valorCelula(s.sku, m, cel.meta);
            vol[m] += v;
            fat[m] += v * (cel.pmv || 0);
        ...
        <div className="px-6 pt-5 pb-3 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs font-medium text-slate-400">
              Ciclo {dados?.ciclo} · defina o Vol. Metas por SKU
            ...
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  a.href = url; a.download = 'metas_preenchimento.xlsx';
                ...
                {meses.map((m) => {
                  ...
                  const fatCat = cat.segmentos.reduce((acc: number, seg: any) =>
                    acc + seg.skus.reduce((a: number, s: any) => {
                      ...
                      return a + valorCelula(s.sku, m, cel.meta) * (cel.pmv || 0);
                    }, 0), 0);
                ...
                <div key={seg.nome} className="ml-4 mt-1">
                  ...
                  {seg.skus.map((s: any) => {
                    ...
                    const div = meses.reduce<'alta' | 'media' | null>((acc, m) => {
                      ...
                      const d = divergenciaIA(cel.ia, valorCelula(s.sku, m, cel.meta));
                    ...
                    return (
                      ...
                      <div
                        ...
                        {meses.map((m) => {
                          ...
                          const val = valorCelula(s.sku, m, cel.meta);
