...
async def obter_status_pipeline():
    return {"is_running": AppState.pipeline_rodando, "logs": AppState.logs}

@router.post("/pipeline/start")
async def iniciar_pipeline(background_tasks: BackgroundTasks):
...
async def reabrir_ciclo(origem: str, db: Session = Depends(get_db)):
