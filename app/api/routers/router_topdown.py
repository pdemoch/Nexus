...
@router.get("/tabela")
def tabela(db: Session = Depends(get_db), u: dict = Depends(require_marketing)):
    """
    ...
    plano = db.execute(text("""
        SELECT f.sku,
               ...
               COALESCE(
                   SUM(f.vol_topdown * f.pmv_aplicado) / NULLIF(SUM(f.vol_topdown), 0),
    ...
    for r in plano:
        ...
        # PMV vem da query, já ponderado por vol_final (padrão único do
        # sistema). Não recalcular por etapa: o preço do SKU não muda
        # conforme a tela que o exibe.
...
def salvar(payload: PayloadSalvar, db: Session = Depends(get_db),
    ...
    try:
        ...
        for aj in payload.ajustes:
            ...
            propagar_linha_jusante(db, ciclo, aj.sku, data_alvo, ETAPA_TOPDOWN)
