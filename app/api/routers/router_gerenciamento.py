...
Endpoints:
  GET  /tabela      -> matriz categoria->segmento->SKU com colunas por mês,
                       IA / BottomUp / realizado-ano-passado, + totalizadores.
  POST /salvar      -> grava vol_bottomup rateado (rascunho). Digitado = somado.
...
# ---------------------------------------------------------------------
@router.get("/status")
def status(db: Session = Depends(get_db), _: dict = Depends(require_gerenciamento)):
    ciclo = get_current_cycle(db)
    return {"ciclo": ciclo, "congelada": etapa_congelada(db, ciclo, ETAPA_BOTTOMUP)}
...
# ---------------------------------------------------------------------
@router.get("/resumo")
def resumo(db: Session = Depends(get_db), _: dict = Depends(require_gerenciamento)):
    """
...
def exportar_visao_geral(
    ...
    _: dict = Depends(require_gerenciamento),
):
    """
...
@router.get("/tabela")
def tabela(db: Session = Depends(get_db), u: dict = Depends(require_gerenciamento)):
    """
    Matriz categoria -> segmento -> SKU, colunas por mês (M2..M4).
    Cada célula-SKU/mês traz: IA, BottomUp (editável), realizado do ano passado
    ...
    plano = db.execute(text("""
        SELECT f.sku,
               ...
               SUM(f.vol_bottomup * f.pmv_aplicado)      AS receita_td
...
# ---------------------------------------------------------------------
@router.get("/exportar")
def exportar(db: Session = Depends(get_db), _: dict = Depends(require_gerenciamento)):
    """
    ...
    segmento, SKU, descrição, mês, IA (cx), vol_bottomup (cx), PMV e
    receita prevista (R$) = vol_bottomup × PMV.
    """
    try:
        ciclo = get_current_cycle(db)
        meses = get_working_window_months(db)
        ...
        registros = []
        for r in rows:
            td = int(r.bottomup or 0)
            receita = float(r.receita_td or 0)
            pmv     = float(r.pmv or 0)   # ponderado por vol_final na query
            registros.append({
        ...
        buffer = io.BytesIO()
        with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
            df.to_excel(writer, index=False, sheet_name="Plano BottomUp")
            # Rodapé informativo
            notas = pd.DataFrame([
                [f"Ciclo: {ciclo}"],
                [f"Meses: {', '.join(m.strftime('%m/%Y') for m in meses)}"],
                ["Receita Prevista = Vol. BottomUp × PMV aplicado"],
                ["Gerado pelo Nexus S&OP — cópia de segurança do preenchimento"],
            ], columns=["Nota"])
            notas.to_excel(writer, index=False, sheet_name="Plano BottomUp",
                           startrow=len(df) + 2, header=False)

        buffer.seek(0)
        nome = f"bottomup_{ciclo.replace('/','_')}.xlsx"
        return StreamingResponse(
            buffer,
            media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
...
# ---------------------------------------------------------------------
@router.get("/dossie")
def dossie(sku: str, db: Session = Depends(get_db), _: dict = Depends(require_gerenciamento)):
    """
...
# ---------------------------------------------------------------------
# POST /salvar  — grava vol_bottomup rateado (rascunho)
# ---------------------------------------------------------------------
@router.post("/salvar")
def salvar(payload: PayloadSalvar, db: Session = Depends(get_db),
           usuario: dict = Depends(require_gerenciamento)):
    """
    Grava cada ajuste em vol_bottomup, rateado com balanço garantido
